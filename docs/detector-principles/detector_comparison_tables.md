# Detector Comparison Tables (Expanded, Reusable)

Reusable, qualitative comparison tables under one set of definitions, expanded to
include specialized and auxiliary detectors. Companion to
`high_energy_detector_principles.md` ("main document"), `glossary.md`, and
`references.md`. Entries give mechanism and scaling, not performance numbers; a
numeric performance claim requires the apparatus, configuration, phase space,
conditions, and source. Abbreviations are defined in `glossary.md`.

**Column conventions.** *Ambiguity* is the characteristic degeneracy of the
inverse problem; *inefficiency/fakes* lists the dominant loss and fake mechanisms;
*validation* names a data-accessible sample (never simulation truth);
*systematics* lists the dominant nuisance parameters.
...
See the mapping in main document Chapter 18, extended here for
...
| Luminosity detectors | coincidence/hit counts | luminosity | linearity, pileup correction, aging (trigger, luminosity, and pileup methodology) |
