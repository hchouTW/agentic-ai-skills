...
class ValidateSkillBundleCliTests(unittest.TestCase):
    ...
    def test_cli_reports_missing_readme_section(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (scratch / "examples/04-decision-tree-multi-skill-ambiguous-routing.md").write_text("# placeholder\n")
            (scratch / "examples/05-trajectory-single-clean-match-academic-papers.md").write_text("# placeholder\n")
            (scratch / "examples/06-trajectory-generic-verb-vs-domain-specific-rule.md").write_text("# placeholder\n")
            (scratch / "examples/07-trajectory-clean-no-match-proceed-normally.md").write_text("# placeholder\n")
            (scratch / "examples/08-gated-pipeline-adding-a-new-rule.md").write_text("# placeholder\n")
            (scratch / "examples/09-gated-pipeline-disambiguating-figure-interpretation.md").write_text("# placeholder\n")
            (scratch / "examples/10-gated-pipeline-expanding-a-carve-out.md").write_text("# placeholder\n")
            ...
            (scratch / "examples/13-adversarial-audit-new-routing-rule-proposal.md").write_text("# placeholder\n")
            (scratch / "examples/14-test-first-routing-parser-primary-secondary.md").write_text("# placeholder\n")
