...
class ExtractRoutedSkillNamesTests(unittest.TestCase):
    def test_extracts_names_from_real_skill_md(self):
        ...
        self.assertEqual(
            names,
            ["academic-diagrams", "academic-papers", "agile-development", "ams-analysis", "deep-learning", "hep-analysis", "task-authoring"],
        )
...
class ValidateSkillBundleCliTests(unittest.TestCase):
    ...
    def test_cli_reports_missing_readme_section(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (scratch / "examples/10-decision-tree-multi-skill-ambiguous-routing.md").write_text("# placeholder\n")
            (scratch / "examples/04-trajectory-single-clean-match-academic-papers.md").write_text("# placeholder\n")
            (scratch / "examples/05-trajectory-generic-verb-vs-domain-specific-rule.md").write_text("# placeholder\n")
            (scratch / "examples/06-trajectory-clean-no-match-proceed-normally.md").write_text("# placeholder\n")
            (scratch / "examples/07-gated-pipeline-adding-a-new-rule.md").write_text("# placeholder\n")
            (scratch / "examples/08-gated-pipeline-disambiguating-figure-interpretation.md").write_text("# placeholder\n")
            (scratch / "examples/09-gated-pipeline-expanding-a-carve-out.md").write_text("# placeholder\n")
            (scratch / "examples/11-decision-tree-figure-interpretation-routing.md").write_text("# placeholder\n")
            (scratch / "examples/12-decision-tree-multiple-false-positive-keywords.md").write_text("# placeholder\n")
            (scratch / "examples/16-adversarial-audit-new-routing-rule-proposal.md").write_text("# placeholder\n")
            (scratch / "examples/19-test-first-routing-parser-primary-secondary.md").write_text("# placeholder\n")
            (scratch / "examples/13-elicitation-new-skill-request.md").write_text("# placeholder\n")
            (scratch / "examples/14-elicitation-two-skills-request.md").write_text("# placeholder\n")
            (scratch / "examples/15-elicitation-router-edge-case-request.md").write_text("# placeholder\n")
            (scratch / "examples/17-adversarial-audit-fallback-text-proposal.md").write_text("# placeholder\n")
            (scratch / "examples/18-adversarial-audit-bullet-consolidation-proposal.md").write_text("# placeholder\n")
            (scratch / "examples/20-test-first-sibling-discovery-invariant.md").write_text("# placeholder\n")
            (scratch / "examples/21-test-first-bundle-validator-message-invariant.md").write_text("# placeholder\n")
            (scratch / "examples/22-postmortem-missing-carve-out-hijack.md").write_text("# placeholder\n")
            (scratch / "examples/23-postmortem-frontmatter-length-truncation.md").write_text("# placeholder\n")
            (scratch / "examples/24-postmortem-generic-verb-double-routing.md").write_text("# placeholder\n")
