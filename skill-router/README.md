...
A portable Agent Skill maintained entirely in English. It contains no scripts or assets - `SKILL.md` is a lightweight triage rule set that routes a task to one of this collection's domain skills (`academic-papers`, `agile-development`, `ams-analysis`, `deep-learning`, `hep-analysis`, `task-authoring`) before work begins. It requires no specific MCP server, cloud account, or paid service.
...
This delivery creates a single folder; it does not change other global agent settings. It only routes to skills that are actually installed - if you don't install `academic-papers`, `agile-development`, `ams-analysis`, `deep-learning`, `hep-analysis`, or `task-authoring`, the corresponding routing row is simply never triggered.
