...
It is explicitly PyTorch-only - not TensorFlow, JAX, or other frameworks - and does not cover general C++ unrelated to LibTorch/custom ops. For broader engineering process (scoping, testing, review), pair it with `agile-development`; for the scientific/statistical validity of a downstream conclusion, pair it with `academic-papers`.
