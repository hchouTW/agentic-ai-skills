...
- A pre-submission pass: checking length limits, undefined references, missing labels

Do **not** reach for this skill for pure numerical/statistical work (fitting, limit
setting, unfolding) — that belongs to `hep-analysis`. Do reach for it the moment that
work needs to be understood from someone else's paper, or turned into English prose,
