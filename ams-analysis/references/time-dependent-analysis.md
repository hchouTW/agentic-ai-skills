...
- **[Documented, abstract level, C36-C42; S41-S47]** AMS reported daily proton, helium, electron and positron fluxes with recurrent 27-day variations (2014-2018 for protons and helium), shorter 13.5-day and 9-day cycles (2016 for protons and helium), strength that depends on time and rigidity (C36, C37, C40); a hysteresis between electron and proton fluxes, and between electron and positron fluxes, below about 8.5 GV (C40, C41); positron variations similar to proton and unlike electron (C41); proton and helium fine structures nearly identical below 40 GV (C38); nuclei He-O with similar but not identical variations whose differences correlate with spectral-index differences (C42). Quote only with the species, range and period in the claim.
- **[Documented, full-text main articles, C43-C49; S41-S47]** How AMS built its time-resolved results: fluxes per day (proton, helium, electron, positron) or per Bartels rotation (proton, helium, nuclei, leptons) with `Φ = N / (A ε T ΔR)`, migration unfolded independently in each time bin, and acceptance, trigger efficiency and collection time per bin (C43, C46, C49); exposure per rotation from livetime-weighted seconds above the geomagnetic cutoff, in normal conditions, within 40° of zenith and outside the South Atlantic Anomaly, with a small time-dependent acceptance correction (C48); systematics split into time-independent and time-dependent parts, the latter (efficiencies evaluated each day) added in quadrature (C44); a wavelet time-frequency analysis with normalized power and 95% confidence levels for periodicities (C45); hysteresis shown by plotting one flux against another with 14-rotation moving averages (C47). The daily electron and positron papers bin in rigidity (1.00-41.9 GV, C40, C41), whereas the Bartels-rotation lepton paper and the time-integrated lepton fluxes bin in energy (C48, C24, C25); state which variable defines your bins. These are the choices of those analyses, not rules.
...
**Evidence gap (not filled here).** The main articles of the time-structure, periodicity and nuclei solar-modulation papers (S41-S47, split out of S16 and S20) were read, but their Supplemental Material and the Phys. Rept. review were not. The ledger therefore does not say how the systematic errors are correlated across days or rotations (the main article gives no covariance model), how the wavelet significance was calibrated, whether any trial-factor or look-elsewhere correction was applied (the main articles give none, which is not proof of absence), how the hysteresis significance was computed, or how the daily collection time is determined. Treat those as **[Unknown/needs input]**; do not supply AMS operational detail. Closing the gap means reading the Supplemental Material and adding scoped claims.
...
- **[Unknown/needs input]** Which method and trial-factor treatment AMS used is documented only in part: a wavelet time-frequency analysis with 95% confidence levels (C45); its calibration, and any trial-factor treatment, are in the Supplemental Material, which was not read. Time-series method references are also not indexed (a Tier 4 gap).
...
Tier 1 AMS papers for what AMS measured and how a specific analysis binned time (S09, S12-S14, S41-S47 main articles; Supplemental Material not read); Tier 4 methodology for trial factors and likelihood construction (S30, S37); heliospheric-modulation models are theory-literature interpretation (Tier 6 unless peer-reviewed methodology).
