...
- `R_ij(η)` migration probability from true bin `j` to reconstructed bin `i`. **State orientation and normalization:** columns (true bins, including any stored under/overflow) sum to one when `R` is conditional on the selected sample and the inefficiency lives in `A_j` (`conditional_on_selected` in `scripts/validate_response.py`), or to the efficiency (at most one) when the inefficiency lives inside `R` (`includes_efficiency`). Do not put inefficiency in both. **A column sum below one has two legitimate readings:** inefficiency (true events not reconstructed in the selected sample), or migration of true events into reconstructed bins outside the tabulated range (under/overflow not stored in `R`); the reverse also holds, since feed-in from outside the true range shows up in row totals. Run `scripts/validate_response.py` on the supplied matrix (and `scripts/validate_covariance.py` for the covariance) before diagnosing. Do not diagnose a sum of, say, 0.85 as inefficiency alone: check whether the matrix stores under/overflow rows, and state which reading the declared metadata implies.
